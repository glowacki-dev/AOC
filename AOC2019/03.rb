...
def perform(t, dx, dy, cross: false)
  t.to_i.times do
    @x += dx
    @y += dy
    @steps += 1
...
cable2.each do |move|
  ...
  case move[0]
  when "R"
    perform(t, 1, 0, cross: true)
  when "L"
    perform(t, -1, 0, cross: true)
  when "U"
    perform(t, 0, 1, cross: true)
  when "D"
    perform(t, 0, -1, cross: true)
  else
    raise StandardError, move
...
r = @crossess.min { |a, b| a[1][0].abs + a[1][1].abs <=> b[1][0].abs + b[1][1].abs }
puts r[1][0] + r[1][1]
